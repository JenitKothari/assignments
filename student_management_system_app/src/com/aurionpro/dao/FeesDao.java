...
public class FeesDao {
	...
	/*
	 * Fees Management View Total Paid Fees View Total Pending Fees View Fees By
	 * Student View Fees By Course Update Fees Of A Course Total Earning Exit
	 */
...
//    }
	...
	public void showFeesByStudent(int studentId) {
		...
		try (PreparedStatement studentPs = conn.prepareStatement(studentQuery);
			...
			if (studentRs.next()) {
				...
				System.out.printf("%-10d | %-20s | %-15s | %-15s | \n", studentRs.getInt("roll_no"),
						studentRs.getString("student_name"), studentRs.getString("mobile_no"),
...
//        Scanner sc = new Scanner(System.in);
		...
		String fetchQuery = "SELECT c.course_id, c.course_name, c.total_fee, sc.paid_fee " + "FROM courses c "
				+ "JOIN student_course sc ON c.course_id = sc.course_id "
		...
		try (PreparedStatement ps = conn.prepareStatement(fetchQuery)) {
			...
			System.out.println("\n=== Enrolled Courses ===");
			System.out.printf("%-10s | %-20s | %-10s | %-10s | %-10s %n", "Course ID", "Course Name", "Total", "Paid",
					"Pending");
			System.out.println("------------------------------------------------------------------------");
			while (rs.next()) {
				found = true;
				...
				System.out.printf("%-10d | %-20s | %-10.2f | %-10.2f | %-10.2f %n", courseId, courseName, total, paid,
						pending);
			}
			...
			try (PreparedStatement pendingStmt = conn.prepareStatement(pendingQuery)) {
				...
				if (result.next()) {
					...
					try (PreparedStatement ps2 = conn.prepareStatement(fetchQuery)) {
						...
						System.out.printf("%-10s | %-20s | %-10s | %-10s | %-10s %n", "Course ID", "Course Name",
								"Total", "Paid", "Pending");
						System.out.println("-----------------------------------------------------------------------");
						while (updatedRs.next()) {
							...
							System.out.printf("%-10d | %-20s | %-10.2f | %-10.2f | %-10.2f %n", courseId, courseName,
									total2, paid2, pending2);
						}
					}
				}
			}
